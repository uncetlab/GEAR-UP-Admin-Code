...
from django.db.models import TextField, Q, F, Count
...
from core.forms import LoginForm, MenuForm, TileForm, PageForm,\
    CollegeForm, UniversityForm, CareerForm
from core.models import School, Menu, CustomPage, Tile, File, Major,\
    College, UniversityProfile, Admin, UserType, AuditVersion, Career
from api.serializers import HomePageSerializer, PageSerializer,\
    FileSerializer, CollegeListSerializer, UniversityListSerializer
...
class SuperAdminBaseView(RevisionMixin, LoginRequiredMixin, View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            if not super_admin:
                return self.user_check_failed(request, *args, **kwargs)
            return super(SuperAdminBaseView, self).dispatch(
                request, *args, **kwargs)
        return redirect(self.login_url + '?next=' + request.get_full_path())
...
class AdminDashBoard(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            context = {'super_admin': super_admin,
...
class DashBoard(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            if university_id and UniversityProfile.objects.filter(
            ...
            if university and university.active:
                ...
                request.session['university_logo'] = university.logo.url \
                    if university.logo else None
...
class LoginView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(
            request,
            self.template_name, {'form': form, 'next': next_page})
...
class ManageSchoolView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'csvfile' in request.FILES:
            ...
            base_headers = ['school-name', 'lea-name', 'address-line-1', 'address-line-2', 'city',
                            'state', 'zip-code', 'latitude', 'longitude']
            header = [slugify(i.value) for i in tuple(ws.rows)[0] if slugify(i.value) in base_headers]
                ...
                if 'school-name' in data:
                    ...
                    try:
                        school, created = School.objects.get_or_create(
                            name=data.get('school-name').strip() if data.get('school-name') else None,
                            lea_name=data.get('lea-name').strip() if data.get('lea-name') else None,
                            address_line1=data.get('address-line-1').strip() if data.get('address-line-1') else '',
                            address_line2=data.get('address-line-2').strip() if data.get('address-line-2') else '',
                            city=data.get('city').strip() if data.get('city') else '',
                            state=data.get('state').strip() if data.get('state') else '',
                            zip_code=str(data.get('zip-code')).strip() if data.get('zip-code') else '',
                            university=self.university,
                            point=Point(float(data.get('longitude', '').strip()),
                                        float(data.get('latitude', '').strip()),
                                        srid=4326) if 'latitude' and
                            'longitude' in data else None,
                        )
            ...
            return HttpResponse(json.dumps({'result': "Success", 'duplicates': duplicates,
                                            'created': new_school, 'errors': skipped}))
        else:
            success = 'true'
            ...
            if not uid and School.objects.filter(
                    name=name,
                    lea_name=lea_name,
            ...
            if uid and School.objects.filter(
                    uid=uid).first():
...
class ManageMenuView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        menus = Menu.objects.filter(
            university=self.university).order_by('order')
        home_page = CustomPage.objects.filter(
            home_page=True, university=self.university).first()
        pages = CustomPage.objects.filter(
            active=True, university=self.university).exclude(
            uid=home_page.uid).order_by('title')
        old_order = dict(Menu.objects.filter(
            university=self.university).annotate(
            str_id=Cast('uid', output_field=TextField())).values_list(
            'str_id', 'order'))
        if request.is_ajax():
            html = render_to_string('menu_list.html',
                                    {'menus': menus, 'pages': pages,
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = request.POST.copy()
        uid = data.get('uid', None)
        if uid and Menu.objects.filter(
                university=self.university, uid=uid).first():
            menu = Menu.objects.filter(
                university=self.university, uid=uid).first()
            order = menu.order
        else:
            order = Menu.objects.filter(
                university=self.university).order_by(
                '-order').first().order + 1 \
    ...
    def delete(self, request, *args, **kwargs):
        ...
        menu.delete()
        Menu.objects.filter(
            university=self.university, order__gte=order).update(
            order=F('order') - 1)
        return HttpResponse('Deleted')
...
class ManageHomePageView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        """Render Template."""
        home_page = CustomPage.objects.filter(
            university=self.university, home_page=True).first()
        print(home_page, self.university)
        pages = CustomPage.objects.filter(
            university=self.university, active=True).exclude(
            uid=home_page.uid).order_by('title')
        tiles = home_page.tiles.all()
        old_order = dict(tiles.annotate(
            str_id=Cast('uid', output_field=TextField())).values_list(
            'str_id', 'order'))
        context = {'home_page': home_page, 'pages': pages, 'tiles': tiles,
                   'old_order': json.dumps(old_order),
                   'video_id': home_page.video_id
                   if home_page.video else None,
                   'video_name': home_page.video.name
                   if home_page.video else None,
                   'video_url': home_page.video.cdn_url
                   if home_page.video else None}
        if request.is_ajax():
            if request.GET.get('request_type') and\
                    request.GET.get('request_type') == 'get_home_page':
                if request.GET.get('uid'):
                    home_page = CustomPage.objects.filter(
                        university=self.university,
                        uid=request.GET.get('uid')).first()
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = request.POST.copy()
        uid = data.get('uid', None)
        if uid and Tile.objects.filter(
                university=self.university, uid=uid).first():
            tile = Tile.objects.filter(
                university=self.university, uid=uid).first()
            order = tile.order
        else:
            order = home_page.tiles.all().\
                order_by('-order').first().order + 1 \
                if home_page.tiles.all().exists() else 0
        tile.order = order
        active = data.get('active')
        ...
        if data.get('type', None) == 'order_update':
            ...
            if form.is_valid():
                ...
                if tile.url_type == "internal":
                    tile.url = None
                if not data.get('image_url', None) and not\
...
class ManageCustomView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        pages = CustomPage.objects.filter(
            university=self.university,
            home_page=False).order_by('title')
        if request.is_ajax():
            if request.is_ajax():
                if request.GET.get('request_type') and\
                        request.GET.get('request_type') == 'get_page':
                    if request.GET.get('uid'):
                        page = CustomPage.objects.get(
                            university=self.university,
                            uid=request.GET.get('uid'))
                        data = PageSerializer(
                            instance=page, html_content=False).data
                        data['related_tiles'] = page.related_tiles
                        return HttpResponse(
                            json.dumps({'page': data}))
            page = request.GET.get('page', 1)
            per_page = request.GET.get('per_page', 10)
            query = request.GET.get('query', None)
            if query:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'bulkDelete' in data:
            ...
            for page in CustomPage.objects.filter(uid__in=multiple_uids):
                page.delete()
                Tile.objects.filter(
                    page=page, university=page.university).delete()
            return HttpResponse('Deleted')
        uid = data.get('uid', None)
        if uid and CustomPage.objects.filter(
                university=self.university, uid=uid).first():
        ...
        if form.is_valid():
            ...
            page = form.save()
            if data.get('file_id', None) and\
                    File.objects.filter(
                        university=self.university,
                        uid=data.get('file_id')).exists():
                page.video = File.objects.get(uid=data.get('file_id'))
            page.save()
            for tile in Tile.objects.filter(
                    page=page, university=page.university):
                if page.active:
...
class ManageMajorsView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        majors = Major.objects.filter(
            university=self.university).order_by('title')
        if request.is_ajax():
            page = request.GET.get('page', 1)
            per_page = request.GET.get('per_page', 10)
            query = request.GET.get('query', None)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'bulkDelete' in data:
            ...
            return HttpResponse('Deleted')
        active = data.get('active')
        if data.get('uid', None) and Major.objects.filter(
                university=self.university,
                uid=data['uid']).first():
            major = Major.objects.filter(
                university=self.university, uid=data['uid']).first()
        else:
        ...
        major.save()
        colleges = []
        if data.get('colleges', None) and College.objects.filter(
                university=self.university,
                uid__in=data['colleges'].split(',')).exists():
            colleges = College.objects.filter(
                university=self.university,
                uid__in=data['colleges'].split(','))
...
class ManageCollegesView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        College.objects.filter(
            university=self.university, active=False).delete()
...
class CollegeDetailView(BaseView):
    ...
    def get(self, request, college_uid=None):
        ...
        if college_uid and College.objects.filter(
                university=self.university,
                uid=college_uid).exists():
            college = College.objects.filter(
                university=self.university, uid=college_uid).first()
        ...
        context = {'majors': majors,
                   'pages': pages, 'tiles': tiles, 'form': form,
                   'college': college,
                   'old_order': json.dumps(old_order),
                   'college_uid': college_uid,
                   'video_id': college.video_id
                   if college and college.video else None,
                   'video_name': college.video.name
                   if college and college.video else None,
                   'video_url': college.video.cdn_url
                   if college and college.video else None}
    ...
    def post(self, request, college_uid):
                ...
                for row in ws.iter_rows(values_only=True):
                    if row[0]:
                        ...
                        if created:
                            ...
                            duplicates += 1
                return HttpResponse(json.dumps({'result': "Success", 'duplicates': duplicates, 'created': new_majors}))
            if data.get('type') == 'add_major':
                major_title = data.get('major_title', None)
                if major_title:
                    major = Major.objects.get_or_create(
                        title=major_title, university=self.university)[0]
                    major.active = True
                    major.save()
                    college.majors.add(major)
                ...
                if tile_uid and Tile.objects.filter(
                        university=self.university, uid=tile_uid).first():
                    tile = Tile.objects.filter(
                        university=self.university, uid=tile_uid).first()
                    order = tile.order
                else:
                    order = college.tiles.all().all().\
                        order_by('-order').first().order + 1 \
                        if college.tiles.all().\
                        exists() else 1
                tile.order = order
        ...
        if 'latitude' in data and 'longitude' in data:
            data['location'] = Point(float(data.get('longitude')),
        ...
        majors = []
        if data.get('major_ids', None) and Major.objects.filter(
                uid__in=data['major_ids'].split(',')).exists():
            majors = Major.objects.filter(
                university=self.university,
                uid__in=data['major_ids'].split(','))
        college.majors.set(majors)
        majors = Major.objects.filter(
            university=self.university, active=True)
        home_page = CustomPage.objects.filter(
            university=self.university, home_page=True).first()
        pages = CustomPage.objects.filter(
            university=self.university, active=True).exclude(
...
class EditProfileView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        """Render Template."""
        base_template = 'base.html' if request.session['front_end'] \
            else 'superadmin/base.html'
        return render(request, self.template_name,
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = request.POST.copy()
        user = request.user
        if data.get('old_password') and \
                not user.check_password(data.get('old_password')):
            return HttpResponse(
                json.dumps({"old_password": "failed",
                            "message": "Incorrect Old Password"}))
        elif data.get('old_password') and \
                user.check_password(data.get('old_password')):
...
class ManageUniversityView(SuperAdminBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.is_ajax():
            html = render_to_string('superadmin/university_list.html',
                                    {'universities': queryset,
                                     'range_list': RANGE_LIST,
                                     'query': query})
            return HttpResponse(html)
        return render(request, self.template_name)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if uid and UniversityProfile.objects.filter(uid=uid).first():
            university = UniversityProfile.objects.filter(
                uid=uid).first()
        else:
        ...
        if form.is_valid():
            ...
            university.save()
            Admin.objects.annotate(
                num_universities=Count('universities')).filter(
                universities=university, num_universities=1).update(active=university.active)
        else:
            HttpResponse(form.errors)
        if not CustomPage.objects.filter(
                university=university, home_page=True).exists():
            home_page = CustomPage(
                university=university, home_page=True,
                active=True, title="Home Page")
            home_page.save()
        for order, u_type in enumerate([
                'Student', 'Teacher', 'Parent', 'Other']):
            if not UserType.objects.filter(
                    title=u_type, university=university).exists():
                user_type = UserType(
                    title=u_type, university=university, order=order)
                user_type.save()
...
class ManageAPICredentialView(SuperAdminBaseView):
    ...
    def generate_random_username(self, length=16,
        ...
        if split:
            username = delimiter.join(
                [username[start:start + split] for start in range(
                    0, len(username), split)])
        try:
    ...
    def get(self, request, *args, **kwargs):
        ...
        university_id = request.GET.get('university', None)
        university = UniversityProfile.objects.filter(
            uid=university_id).first()
        credentials = Admin.objects.filter(
            universities=university,
            role='silent_login').values(
            'user__username', 'user_id', 'silent_login_password')
        html = render_to_string('superadmin/credentials_list.html',
    ...
    def post(self, request, *args, **kwargs):
        ...
        if Admin.objects.filter(
                universities=university, role='silent_login').count() < 2:
            username = self.generate_random_username(length=8)
            password = self.generate_random_username(
                length=16, delimiter='')
            ...
            api_user.save()
            admin = Admin(user=api_user,
                          role='silent_login', active=True,
        ...
        html = render_to_string('superadmin/credentials_list.html',
                                {'credentials': credentials})
        return HttpResponse(json.dumps({
            'success': 'true', 'html': html, 'uid': str(university.uid)}))
...
class ManageAdminView(SuperAdminBaseView):
    ...
    def get(self, request, *args, **kwargs):
        """Render Template."""
        queryset = Admin.objects.all().exclude(
            role='silent_login').order_by('user__username')
        page = request.GET.get('page', 1)
        per_page = request.GET.get('per_page', 10)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if resend_verification and resend_verification == 'true':
            ...
            return HttpResponse(json.dumps({
                'success': 'true',
                'message': 'Verification link resend to admin'}))
        ...
        universities = []
        if data.get('university_ids', None) and \
            UniversityProfile.objects.filter(
                uid__in=university_ids.split(',')).exists():
...
class ManageLogView(SuperAdminBaseView):
    ...
    def get(self, request, *args, **kwargs):
        """Render Template."""
        queryset = AuditVersion.objects.all().order_by(
            '-revision__date_created').exclude(
            content_type=ContentType.objects.get_for_model(UserType))
        page = request.GET.get('page', 1)
        per_page = request.GET.get('per_page', 10)
        ...
        if query:
            ...
            if university:
                universities_list = [
                    ...
                    if item.object and hasattr(item.object, 'university') and item.object.university == university]
        ...
        if export_csv:
            ...
            for row in queryset:
                wr.writerow([row.revision.date_created.strftime("%m/%d/%Y-%H:%M:%S"),
                             ...
                             '-- '.join(
                             ['%s: %s' % (c['field'].name,
                              BeautifulSoup(c['diff'], "html.parser").text.replace('\n', '  ')) for c in row.changes])])
...
class ManageCareersView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        queryset = Career.objects.filter(
            university=self.university).order_by('career')
        page = request.GET.get('page', 1)
        per_page = request.GET.get('per_page', 10)
        query = request.GET.get('query', '')
        if query:
...
class CareerDetailView(BaseView):
    ...
    def get(self, request, career_uid=None):
        ...
        if career_uid and Career.objects.filter(
                university=self.university,
                uid=career_uid).exists():
            career = Career.objects.filter(
                university=self.university, uid=career_uid).first()
            form = CareerForm(instance=career)
            tiles = career.tiles.all().order_by('order')
            old_order = dict(tiles.annotate(
                str_id=Cast('uid', output_field=TextField())).values_list(
                'str_id', 'order'))
        context = {'tiles': tiles, 'form': form,
                   'career': career,
                   'old_order': json.dumps(old_order),
                   'career_uid': career_uid,
                   'video_id': career.video_id
                   if career and career.video else None,
                   'video_name': career.video.name
                   if career and career.video else None,
                   'video_url': career.video.cdn_url
                   if career and career.video else None}
