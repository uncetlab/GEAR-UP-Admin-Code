...
class Admin(models.Model):
    ...
    active = models.BooleanField(default=True)
    role = models.CharField(choices=ROLES, default="admin", max_length=100)
    silent_login_password = models.CharField(null=True, blank=True, max_length=16)
    ...
    def to_json(self):
        ...
        return {
            ...
            "universities": list(
                self.universities.annotate(
                    str_id=Cast("uid", output_field=models.TextField())
    ...
    def formatted_universities(self):
        ...
        if self.universities.all():
            return ", ".join(self.universities.all().values_list("name", flat=True))
        return ""
...
class Device(models.Model):
    ...
    user_type = models.ForeignKey(
        UserType, null=True, blank=True, on_delete=models.CASCADE
    )
...
class Menu(models.Model):
    ...
    url_type = models.CharField(choices=TYPES, default="native", max_length=100)
...
class School(models.Model):
    ...
    def address_formatted(self):
        ...
        return ", ".join(filter(None, [self.address_line1, self.address_line2]))
...
class Tile(models.Model):
    ...
    TYPES = [("native", "Native"), ("external", "External")]
    uid = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    title = models.CharField(max_length=100)
    url = models.CharField(max_length=500, null=True, blank=True)
    active = models.BooleanField(default=True)
...
class CustomPage(models.Model):
    ...
    video = models.ForeignKey(
        "File", null=True, blank=True, on_delete=models.SET_NULL, related_name="page"
    )
    ...
    def related_tiles(self):
        ...
        return (
            ...
            if Tile.objects.filter(
                page=self, active=True, university=self.university
            ).exists()
            else False
        )
    ...
    def dark_mode_content(self):
        ...
        default_template = template_name.replace(
            slugify(self.university.name), "default"
        )
...
class Major(models.Model):
    ...
    def college_names(self):
        ...
        return ", ".join(list(self.college_set.all().values_list("name", flat=True)))
...
class College(models.Model):
    ...
    video = models.ForeignKey(
        "File", null=True, blank=True, on_delete=models.SET_NULL, related_name="college"
    )
    logo = models.FileField(null=True, blank=True)
    ...
    def html_content(self, base_url=""):
        ...
        for tile in self.tiles.filter(active=True):
            ...
            %(tile_title)s
                ...
                if tile.image
                else base_url + "/static/images/default_tile_img-s.png",
    ...
    def dark_mode_content(self, base_url=""):
        ...
        for tile in self.tiles.filter(active=True):
            ...
            %(tile_title)s
                ...
                "tile_icon": base_url + tile.image.url
                if tile.image
                else "/static/images/default_tile_img-s.png",
                "tile_title": tile.title,
                "img_style": "max-width: 100%; max-height:100%; vertical-align: -webkit-baseline-middle",
            }
        context = {"content": self.content, "tile_html": tile_html}
        template_name = "themes/%s/dark_mode.html" % slugify(self.university.name)
        default_template = template_name.replace(
            slugify(self.university.name), "default"
        )
    ...
    if user:
        ...
        if new_password != old_password:
            if Admin.objects.filter(user=user, verified=False).exists():
                Admin.objects.filter(user=user, verified=False).update(verified=True)
...
class AuditVersion(Version):
    ...
    def status(self):
        ...
        if not self.object:
            return "deleted"
        if (self.revision.date_created - self.object.created) < timedelta(seconds=10):
            return "created"
        return "modified"
    ...
    def changes(self):
        ...
        if self.prev:
            return CompareMixin.compare(CompareMixin(), self.object, self.prev, self)[0]
        return []
    ...
    def prev(self):
        ...
        if self.object:
            return (
                AuditVersion.objects.get_for_object(self.object)
                .exclude(pk=self.pk)
...
class Career(models.Model):
    ...
    modified = models.DateTimeField(auto_now=True)
    video = models.ForeignKey(
        "File", null=True, blank=True, on_delete=models.SET_NULL, related_name="career"
    )
    logo = models.FileField(null=True, blank=True)
    tiles = models.ManyToManyField(Tile, blank=True)
    university = models.ForeignKey(UniversityProfile, on_delete=models.CASCADE)
