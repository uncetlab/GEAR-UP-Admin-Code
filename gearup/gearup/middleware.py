...
class LoggingMiddleware(object):
    ...
    def _get_logging_context(self, request, response):
        ...
        Return a map with args and kwargs to provide
        additional context to calls to logging.log().

        This allows the logging context to be created
    ...
    def _log_multipart(self, body, logging_context):
        ...
        Split multipart body into parts separated by "boundary",
        then matches each part to BINARY_REGEX.

        which searches for existence of "Content-Type" and
        capture of what type is this part.
        If it is an image or an application replace that
        content with "(binary data)" string.
        This function will log "(multipart/form)" if body can't
        be decoded by utf-8.
        """
        try:
    ...
    def _log_resp(self, level, response, logging_context):
        if re.match(
            ...
            if response.streaming:
                ...
                # it's because large and it might hit
                # the max_body_length very often. Not to mention that
