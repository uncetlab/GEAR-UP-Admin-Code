...
from core.models import UserType, School, CustomPage,\
    College, UniversityProfile, Admin
...
class TokenGenerationTests(APITestCase):
    ...
    def test_current_user(self):
        ...
        user = User.objects.create_user(username='sherlock',
        ...
        university = UniversityProfile.objects.get_or_create(
            name="NC University", active=True)[0]
        ...
        response = self.client.get(reverse('user_type_list'),
                                   data={'format': 'json'})
        self.assertEqual(response.status_code, status.HTTP_200_OK,
                         response.content)
...
class ObtainTokenBase(APITestCase):
    ...
    def obtain_token(self):
        ...
        admin = Admin.objects.get_or_create(
            user=user, role='silent_login', active=True)[0]
        admin.save()
        admin.universities.set([university])
        admin.save()
        self.assertEqual(user.is_active, 1, 'Active User')
...
class CreateNewDeviceTest(ObtainTokenBase):
    ...
    def setUp(self):
        ...
        user_type = UserType.objects.get_or_create(
            title='Student', university=university)[0]
        school = School.objects.get_or_create(
            name='RajaGiri', university=university)[0]
        self.valid_payload = {
...
class CustomPageTest(ObtainTokenBase):
    ...
    def test_custom_page(self):
        """Test for user list endpoint."""
        token, university = self.obtain_token()
        page = CustomPage.objects.get_or_create(
            title="Sample", university=university)[0]
        self.client.credentials(HTTP_AUTHORIZATION='Bearer {0}'.format(token))
        response = self.client.get(reverse('custom_page',
...
class CollegeDetailTest(ObtainTokenBase):
    ...
    def test_user_type_list(self):
        ...
        college = College(
            ...
            address='Atlanta, GA',
            location=Point(float('-90.3084017323959'),
                           float('38.64724015'), srid=4326),
            university=university)
        college.save()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer {0}'.format(token))
        response = self.client.get(reverse('college_detail',
                                   kwargs={'college_uid': college.uid}),
                                   data={'format': 'json'})
        self.assertEqual(response.status_code, status.HTTP_200_OK,
                         response.content)
