...
class BaseView(generics.GenericAPIView):
    ...
    def initial(self, request, *args, **kwargs):
        ...
        if request.user.is_superuser:
            ...
            if university and UniversityProfile.objects.filter(uid=university).exists():
...
class SchoolListView(BaseView):
        ...
        if query:
            ...
            if name and address_1:
                queryset = School.objects.filter(
                    Q(name__icontains=name) | Q(address_line1__icontains=address_1)
                ).order_by("name")
            elif address_2:
                queryset = School.objects.filter(
                ...
                queryset = School.objects.filter(
                    Q(name__icontains=name) | Q(address_line1__icontains=name)
                ).order_by("name")
        else:
    ...
    def post(self, request):
        ...
        data["app_info"] = request.META.get("HTTP_USER_AGENT") or data.get("app_info")
...
class HomePageView(BaseView):
    ...
    def get(self, request, format=None):
        ...
        home_page = CustomPage.objects.filter(
            home_page=True, university=admin.university
        ).first()
...
class FilePolicyAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (
            ...
            return Response(
                {"message": "University needs to be specified"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if replace_video_id and File.objects.filter(uid=replace_video_id).exists():
        ...
        file_obj = File.objects.create(name=fname, university=university)
        file_obj_id = file_obj.uid
        upload_start_path = self.check_s3_key_exists(
            fname, file_extension, str(file_obj_id)
        )
        ...
        filename_final = "{file_obj_id}{file_extension}".format(
            file_obj_id=file_obj_id, file_extension=file_extension
        )
...
class FileUploadCompleteHandler(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if obj_type and obj_id:
            if (
                obj_type == "custom_page"
                and CustomPage.objects.filter(uid=obj_id).exists()
            ):
                page = CustomPage.objects.get(uid=obj_id)
                page.video = obj
                page.save()
            elif obj_type == "college" and College.objects.filter(uid=obj_id).exists():
...
class CollegeListView(BaseView):
    ...
    def post(self, request):
        ...
        if filters:
            ...
            state_type = filters.get("state_type", "in_state")
            ...
            disability_access = filters.get("disability_access", None)
            ...
            if bool(sat_score_range):
                if "min" not in sat_score_range or "max" not in sat_score_range:
                    raise ErrorBadParameters(
            ...
            if bool(act_score_range):
                if "min" not in act_score_range or "max" not in act_score_range:
                    raise ErrorBadParameters(
            ...
            if bool(avg_cost_range):
                if "min" not in avg_cost_range or "max" not in avg_cost_range:
                    raise ErrorBadParameters(
            ...
            if bool(student_capacity):
                if student_capacity["operator"] == "bt":
                    if (
                        "value1" not in student_capacity
                    ...
                    if "value1" not in student_capacity:
                        raise ErrorBadParameters("Specify `value1` in student_capacity")
                    queryset = queryset.filter(
            ...
            if bool(location_poly):
                if (
                    "lat1" not in location_poly
                    or "lng1" not in location_poly
            ...
            if disability_access:
                queryset = queryset.filter(
                    disability_access=True, disability_access_url__isnull=False
                )
